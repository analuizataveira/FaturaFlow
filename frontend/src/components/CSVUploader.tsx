...
import { uploadCsvInvoices } from '../services/InvoiceService';
...
const CsvUploader = ({ userId, onUploadSuccess }: CsvUploaderProps) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      const result = await uploadCsvInvoices(userId, file);
      
      setMessage(`Sucesso! ${result.invoicesCreated || 0} faturas foram criadas.`);
      setFile(null);
      resetFileInput();
