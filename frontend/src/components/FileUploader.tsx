...
const FileUploader = ({ userId, onUploadSuccess }: FileUploaderProps) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      let result;
      if (uploadType === "csv") {
        result = await uploadCsvInvoices(userId, file);
      } else {
        result = await uploadPdfInvoices(userId, file);
      }
        ...
        <label htmlFor="fileInput" className="cursor-pointer block">
          <div className="flex flex-col items-center justify-center">
            ...
            <p className="text-sm text-gray-500">
              Apenas arquivos {uploadType.toUpperCase()} são aceitos
            </p>
          </div>
        </label>
      </div>
